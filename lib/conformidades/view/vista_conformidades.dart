...
class ConformidadesPage extends StatefulWidget {
  ...
  @override
  State<ConformidadesPage> createState() => _ConformidadesPageState();
}

class _ConformidadesPageState extends State<ConformidadesPage> {
  String filter = '';
  int endList = 70;
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<MainBloc, MainState>(
      builder: (context, state) {
        ...
        List<ToCelda> titulos = state.lclConfList?.celdas ?? [];
        List<ConformidadesReg> valores = state.lclConfList?.list ?? [];
        valores = valores
            ...
            .toList();
        List<ConformidadesReg> valoresToShow = [];
        ...
        return Scaffold(
          appBar: AppBar(
            title: const Text("CONFORMIDADES (Millones de pesos)"),
            actions: [
              const Gap(5),
              ElevatedButton(
                onPressed: () {
                  ...
                  DescargaHojas().ahoraMap(
                    datos: datos,
                    nombre: "Conformidades ${widget.contrato}",
                  );
                },
                child: const Text(
                  'Descargar',
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (c, index) {
                        ...
                        return InkWell(
                          ...
                          child: Container(
                            color: valoresToShow[index].importe < 0
                                ? Colors.red[100]
                                : null,
                            child: Row(
                              key: UniqueKey(),
